...
class AttributesObserver {
   ...
   */
  public function __construct(IndexerManager $indexerManager, WizzyProduct $wizzyProduct, ProductsManager $productsManager) {
    $this->indexer = $indexerManager->getProductsIndexer();
    $this->wizzyProduct = $wizzyProduct;
    $this->productsManager = $productsManager;
  }

  public function afterSave(AttributeInterceptor $attributeInterceptor, AttributeInterceptor $attributeInterceptorResult, EavAttributeInterceptor $attribute) {
  ...
  public function beforeDelete(AttributeInterceptor $attributeInterceptor, EavAttributeInterceptor $attribute) {
    $productIdsToUpdate = $this->getEffectedProductIds($attribute);
    $this->addProductsInSync($productIdsToUpdate);
  }
  ...
  private function getEffectedProductIds(EavAttributeInterceptor $attribute) {
    $productIds = [];
    $products = $this->productsManager->getProductsByAttribute($attribute->getAttributeCode(), $attribute->getStoreId());
