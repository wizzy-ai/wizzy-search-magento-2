...
class ProductsMapper {
   ...
   public function __construct(Configurable $configurable, ProductsManager $productsManager, ConfigurableProductsData $configurableProductsData, AttributesManager $attributesManager, StockRegistry $stockRegistry) {
   ...
   public function map($product) {
      ...
      if ($mappedProduct['mainImage'] == "" || empty($mappedProduct['categories']) || empty($mappedProduct['sellingPrice']) || $mappedProduct['sellingPrice'] == 0) {
         ...
         foreach ($children as $child) {
            if ($finalPrice < $child->getFinalPrice()) {
               ...
               if ($child->getFinalPrice() < $child->getPrice()) {
                  ...
                  $discountPercetnage = 100 - sprintf('%0.2f', (($child->getFinalPrice() * 100) / $child->getPrice()));
            ...
            if ($child->getFinalPrice() && $child->getFinalPrice() > 0 && $child->getPrice() > 0) {
               if ($child->getFinalPrice() < $child->getPrice()) {
                  $mappedProduct['childData']['discounts'][] = $this->getFloatVal($child->getPrice() - $child->getFinalPrice());
                  $mappedProduct['childData']['discountPercentages'][] = $this->getFloatVal(100 - sprintf('%0.2f', (($child->getFinalPrice() * 100) / $child->getPrice())));
               }
            }
            ...
            if (!$mappedProduct['isSearchable']) {
               $mappedProduct['isSearchable'] = ($childVisibility == Visibility::VISIBILITY_IN_SEARCH || $childVisibility == Visibility::VISIBILITY_BOTH) ? TRUE : $mappedProduct['isSearchable'];
            }

            if (!$mappedProduct['isVisibleInCatalog']) {
               $mappedProduct['isVisibleInCatalog'] = ($childVisibility == Visibility::VISIBILITY_IN_CATALOG || $childVisibility == Visibility::VISIBILITY_BOTH) ? TRUE : $mappedProduct['isVisibleInCatalog'];
            }
            ...
            $colors = array_merge($colors, $childColors);
            $sizes = array_merge($sizes, $childSizes);
   ...
   private function mapAttributes($product, &$mappedProduct, $variationInStock, $isForChild = FALSE) {
      ...
      foreach ($product->getAttributes() as $attribute) {
         ...
         if ($isUserDefined && ($isFilterableInSearch || $isFilterable || $isSearchable) && (count($value) > 1 || (count($value) == 1 && !empty($value[0])))) {
            $autocompleteConfig = $this->getAutocompleteConfig($attribute, $autocompleteAttributes);
            $attributeValueToAdd = [
               'value' => $value,
      ...
      if (count($parentProductIds)) {
         ...
         if (count($parentProducts)) {
            foreach ($parentProducts as $parentProduct) {
               ...
               $mappedProduct['isSearchable'] = ($visibility == Visibility::VISIBILITY_IN_SEARCH || $visibility == Visibility::VISIBILITY_BOTH) ? TRUE : FALSE;
               $mappedProduct['isVisibleInCatalog'] = ($visibility == Visibility::VISIBILITY_IN_CATALOG || $visibility == Visibility::VISIBILITY_BOTH) ? TRUE : FALSE;
               break;
            }
         }
      }
   ...
   private function mapBasicDetails($product, &$mappedProduct) {
      ...
      $mappedProduct = [
         ...
         'isSearchable' => ($visibility == Visibility::VISIBILITY_IN_SEARCH || $visibility == Visibility::VISIBILITY_BOTH) ? TRUE : FALSE,
         'isVisibleInCatalog' => ($visibility == Visibility::VISIBILITY_IN_CATALOG || $visibility == Visibility::VISIBILITY_BOTH) ? TRUE : FALSE,
      ];
   ...
   private function mapDiscounts($product, &$mappedProduct) {
      if ($product->getFinalPrice() && $product->getFinalPrice() > 0 && $product->getPrice() > 0) {
         if ($product->getFinalPrice() < $product->getPrice()) {
            ...
            $mappedProduct['discountPercentage'] = $this->getFloatVal(100 - sprintf('%0.2f', (($product->getFinalPrice() * 100) / $product->getPrice())));
