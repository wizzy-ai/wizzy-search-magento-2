...
class IndexProductsProcessor extends QueueProcessorBase {
   ...
   public function __construct(ProductsManager $productsManager, OrderItemManager $orderItemManager, StoreGeneralConfig $storeGeneralConfig, EntitiesSync $entitiesSync, Products $productsConnector, ProductsMapper $productsMapper, ReviewRatingsManager $reviewRatingsManager) {
   ...
   private function findDeletedAndInactiveProducts($productIds, $products) {
      ...
      foreach ($products as $product) {
         if (!$product->isDisabled() && $product->getVisibility() != strval(Visibility::VISIBILITY_IN_CATALOG)) {
