...
class IndexPagesProcessor extends QueueProcessorBase {
   ...
   public function __construct(StoreManager $storeManager, StoreGeneralConfig $storeGeneralConfig, PagesManager $pagesManager, StoreAutocompleteConfig $storeAutocompleteConfig, Pages $pages) {
   ...
   private function getPagesToDelete($pagesToSave, $storeId) {
      ...
      $pagesToDelete = [];
      if ($addedPages !== FALSE && count($pagesToSave)) {
      ...
      foreach ($pages as $page) {
         ...
         if (in_array($storeId, $stores) || in_array("0", $stores)) {
            $pagesToSave[] = [
               ...
               'isExcluded' => (in_array($page->getId(), $pagesToExclude) || in_array(0, $pagesToExclude)) ? TRUE : FALSE,
            ];
         }
      }
